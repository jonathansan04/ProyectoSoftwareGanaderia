const typeCollections = {
    ...
    { field: 'extension', type: 'number', required: false },
    { field: 'tipopasto', type: 'text', length: 50, required: true },
    { field: 'idFinca', type: 'text', length: 20, required: true }],
    ...
    { field: 'apellido', type: 'text', length: 50, required: true },
    { field: 'celular', type: 'text', length: 15, required: true },
    { field: 'tipoid', type: 'text', length: 3, required: true },
    { field: 'numero_id', type: 'text', length: 20, required: true },
  ...
  validate = (col, obj) => typeCollections[col].every(({ field, type, length, required }) => // Check if all required keys are present
    (required && !obj[field]) || (length && obj[field].length > length) ? false : true),
  invalidate = (col, obj) => Object.keys(obj).some(key => // Check if any key is not present in the base collection
    !typeCollections[col].map(({ field }) => field).includes(key)),
  collections = {};
